...
class ClienteModel
{
    ...
    private $alias = "cl"; // Alias de la tabla referente al modelo
    private $primaryKey = "id_cliente";
    private $db;
    private $fecha;
    private $OrderBy = "fecha_registro";
    ...
    public function ClienteAdd(array $data)
    {
        ...
        try {
            ...
            $stmt = $this->db->prepare("INSERT INTO `{$this->tabla}` (nombre, apellidos, direccion, telefono, email, fecha_registro) 
                                        VALUES (:NOMBRE, :APELLIDOS, :DIRECCION, :TELEFONO, :CORREO, :FECHA)");
            $stmt->bindValue(":NOMBRE", strtoupper($data["nombre"]));
            $stmt->bindValue(":APELLIDOS", strtoupper($data["apellido"]));
            $stmt->bindValue(":DIRECCION", $data["direccion"]);
            $stmt->bindValue(":TELEFONO", "+503 " . $data["telefono"]);
            $stmt->bindValue(":CORREO", $data["correo"]);
            $stmt->bindValue(":FECHA", date('Y-m-d H:i:s')); // Suponiendo que estás añadiendo la fecha actual
            ...
            // Inserción de datos del vehículo
            $anotherStmt = $this->db->prepare("INSERT INTO `vehiculos` (id_cliente, marca, modelo, año, placa, serieMotor) 
                                               VALUES (:ID_CLIENTE, :MARCA, :MODELO, :ANO, :PLACA, :SMOTOR)");
            // Vinculación de los valores a los marcadores de posición
            $anotherStmt->bindValue(":ID_CLIENTE", intval($insertedId));
            $anotherStmt->bindValue(":MARCA", strtoupper($data["marca"]));
            $anotherStmt->bindValue(":MODELO", strtoupper($data["modelo"]));
            $anotherStmt->bindValue(":ANO", $data["year"]);
            $anotherStmt->bindValue(":PLACA", strtoupper($data["placa"]));
            $anotherStmt->bindValue(":SMOTOR", strtoupper($data["SerieMotor"]));
            $anotherStmt->execute();
