...
document.getElementById('formAddMarca').addEventListener('submit', async function(event) {
  event.preventDefault(); // Evita el envío del formulario por defecto
  const form = event.target;
  const url = form.getAttribute('data-add');
  const formData = new FormData(form);
  const api = new FetchAPI(url)
  ...
  try {
      ...
      if (data.status == "success") {
        ...
        // Aquí puedes agregar las funciones que necesitas llamar tras actualizar los datos
        setTimeout(() => {
          document.getElementById('cerrarbtn').click();
        }, 1000);
      } else {
